...
//Завдання 3: Створіть функцію, що повертає новий об'єкт з тими ж властивостями, що й 
//переданий у якості аргумента об'єкт.
...
function copyObject(obj) {
  const objNew = {...obj}// Використовуємо синтаксис деструктурізації {...person} для створення нового об'єкта з тими ж властивостями
  return objNew// Повертаємо новий об'єкт
}
...
function hasProperty(obj, property) {
  if (property in obj) {// Використовуємо оператор "in" для перевірки наявності властивості
  return `Property ${property} exists`}// Запишимо умову якщо властивість існує повертає текст Property ${property} exists,
  return `Property ${property} does not exist`// інашке повертаємо Property ${property} does not exist.
}
...
function printKeysAndValues(obj) {
  for (const a in obj) {// Проходимося по всіх ключах об'єкту за допомогою циклу "for in"
  console.log(`Key${a}, Value: ${obj[a]}`)// Виводимо ключ та значення на консоль
  }
}
...
function addField(obj, newField, value) {
  obj[newField] = value// Додаємо нове поле до об'єкту з допомогою квадратних дужок
  return obj// Повертаємо об'єкт
}
...
function destructureObject(obj) {
  let {brand, model} = obj// Використовуємо деструктуризацію для створення нових змінних з властивостей об'єкту і отримуємо з нього змінні brand та model
...
function changeRole(array, newRole) {
  for (const a of array) {// Ітеруємося по масиву об'єктів за допомогою циклу "for of"
  a.role = newRole// Змінюємо роль кожного користувача на нове ім'я
  console.log(a)// Виводимо об'єкт на консоль
  }
}
...
function printProductDetails(obj) {
  console.log("Деталей людини для виводу не виявлено, тільки це:")// Використовуємо деструктуризацію для отримання значень productName, price i також значень companyName, country вкладеного об'єкту manufacturer
  let {
    productName,
    price,
    manufacturer: { companyName, country },
  } = obj; // Використовуємо деструктуризацію для отримання значень productName, price i також значень companyName, country вкладеного об'єкту manufacturer
...
function compareObjects(obj1, obj2) {
  ...
  obj2 = obj1// Присвоємо obj2 значення об'єкту obj1
  console.log(obj1 === obj2)// Виводимо результат порівняння об'єктів
}
...
function addProperty(array) {
  ...
  return newArray// Повертаємо переданий масив з новою властивістю
}
